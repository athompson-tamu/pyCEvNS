...
class NeutrinoFluxFactory:
    def __init__(self):
        self.flux_list = ['solar', 'solar_b8', 'solar_f17', 'solar_hep', 'solar_n13', 'solar_o15', 'solar_pp',
                          ...
                          'far_beam_nu', 'far_beam_nubar', 'atmospheric', 'atmospheric_extended',
                          'atmospheric_sub100mev']

    def print_available(self):
        print(self.flux_list)

    def get(self, flux_name, modulation=1.0, **kwargs):
        ...
        if flux_name == 'solar':
            ...
            be7_862 = modulation * 4.93e9
            be7_384 = modulation * 5.3e8
            return NeutrinoFlux(continuous_fluxes={'ev': f[:, 0], 'e': f[:, 1]},
                                delta_fluxes={'e': [(1.439, 1.44e8), (0.862, be7_862), (0.384,be7_384)]}) # using HZ-SSM
        if flux_name == 'pep':
            return NeutrinoFlux(delta_fluxes={'e': [(1.439, 1.44e8), ]})
        if flux_name == 'be7':
            return NeutrinoFlux(delta_fluxes={'e': [(0.862, 4.93e9),(0.384,5.3e8)]})
        ...
        if flux_name == 'atmospheric':
            ...
            return NeutrinoFlux(continuous_fluxes=nu)
        if flux_name == 'atmospheric_extended':
            if 'zenith' not in kwargs:
                raise Exception('please specify zenith angle')
            zen = np.round(kwargs['zenith'], decimals=3)
            zen_list = np.round(np.linspace(-0.975, 0.975, 40), decimals=3)
            if zen not in zen_list:
                ...
                raise Exception('zenith angle not available')
            idx = (0.975 - zen) / 0.05 * 61
            f_atmos = np.genfromtxt(pkg_resources.resource_filename(__name__, 'data/atmos_extrapolated.txt'),
                                    delimiter=',')
            nu = {'ev': f_atmos[int(round(idx)):int(round(idx))+80, 0],
                  'e': f_atmos[int(round(idx)):int(round(idx))+80, 2],
                  'mu': f_atmos[int(round(idx)):int(round(idx))+80, 3],
                  'ebar': f_atmos[int(round(idx)):int(round(idx))+80, 5],
                  'mubar': f_atmos[int(round(idx)):int(round(idx))+80, 6]}
            return NeutrinoFlux(continuous_fluxes=nu)
        if flux_name == 'atmospheric_sub100mev':
            atmos_txt = 'data/atmos_sub_100_mev.txt'
            f_atmos = np.genfromtxt(pkg_resources.resource_filename(__name__, atmos_txt), delimiter=',')
            nu = {'ev': f_atmos[:, 0],
                  'e': f_atmos[:, 1],
                  'mu': f_atmos[:, 2],
                  'ebar': f_atmos[:, 4],
