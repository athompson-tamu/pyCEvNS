...
def EventsGenerator(nsi_array, expo, flux, osc_factory):
  ...
  nsi = NSIparameters(0)
  nsi.epel = {'ee': nsi_array[0], 'mm': nsi_array[2], 'tt': nsi_array[4], 'em': nsi_array[6], 'et': nsi_array[8],
              'mt': nsi_array[10]}
  nsi.eper = {'ee': nsi_array[1], 'mm': nsi_array[3], 'tt': nsi_array[5], 'em': nsi_array[7], 'et': nsi_array[9],
              'mt': nsi_array[11]}
...
def FlatPrior(cube, ndim, nparams):
  ...
  cube[6] = 0.1 * (2 * cube[6] - 1)  # eps_emu L
  cube[7] = 0.1 * (2 * cube[7] - 1)  # eps_emu R
  cube[8] = 0.1 * (2 * cube[8] - 1)  # eps_eta L
  cube[9] = 0.1 * (2 * cube[9] - 1)  # eps_eta R
  cube[10] = 0.1 * (2 * cube[10] - 1)  # eps_muta L
  cube[11] = 0.1 * (2 * cube[11] - 1)  # eps_muta R
...
def main():
  ...
  parameters = ["epsl_ee", "epsr_ee", "epsl_mumu", "epsr_mumu", "epsl_tautau", "epsr_tautau",
                "epsl_emu", "epsr_emu", "epsl_etau", "epsr_etau", "epsl_mutau", "epsr_mutau", "delta_cp"]
