...
flux_target = np.array(list(zip(flux_bins,flux_hist*0.95)))
flux_dump = np.array(list(zip(flux_bins,flux_hist*0.05)))
...
def SandwichSearch(generator, mass_array, g_array):
    ...
    for i in range(mass_array.shape[0]):
        ...
        for g in g_array:
            ...
            ev *= 4*np.pi*generator.detector_distance**2 * 0.05
            sig = np.sqrt(ev)
            if sig > sig_limit:
                lower_array[i] = g
                break
    ...
    np.savetxt("limits/DUNE/preliminary_limits_isotropic_v0.txt", limits_array)
    return limits_array
...
def main():
    axion_gen_target = IsotropicAxionFromPrimakoff(photon_rates=flux_target, axion_mass=1, axion_coupling=1e-6,
                                                   target_mass=12e3, target_z=6, target_photon_cross=1e-24,
                                                   detector_distance=574, detector_area=21, detector_length=10)
    axion_gen_dump = IsotropicAxionFromPrimakoff(photon_rates=flux_dump, axion_mass=1, axion_coupling=1e-6,
                                                 target_mass=28e3, target_z=14, target_photon_cross=1e-24,
                                                 detector_distance=304, detector_area=21, detector_length=10)
    ...
    if rerun == True:
      ...
      g_array = np.genfromtxt("limits/DUNE/preliminary_limits_isotropic_v0.txt")
