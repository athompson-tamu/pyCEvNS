...
for i in range(0,energy_bins.shape[0]):
    for j in range(len(timing_bins)):
        ...
        n_prompt[i] = efficiency(energy_bins[i]*pe_per_mev) * \
                      binned_events_electron(e_a, e_b, exposure, Detector('ar'), prompt_flux, NSIparameters(),
                                            flavor='mu', rn=5.5) * prompt_time(timing_bins[j])
        n_delayed[i] = efficiency(energy_bins[i]*pe_per_mev) * \
                       (binned_events_electron(e_a, e_b, exposure, Detector('ar'), delayed_flux, NSIparameters(),
                                              flavor='e', rn=5.5)
                        + binned_events_electron(e_a, e_b, exposure, Detector('ar'), delayed_flux, NSIparameters(),
...
def EventsGen2(cube):
    ...
    flux = DMFluxIsoPhoton(photon_flux, dark_photon_mass=dp_mass, coupling=1, dark_matter_mass=dp_mass/3,
                           ...
                           verbose=False)
    dm_gen_2.fx = flux
    return dm_gen_2.events(cube[0], cube[1], n_meas, channel="electron")  # mediator mass, epsilon
...
def main():
    ...
    if use_save == True:
        ...
        saved_limits = np.genfromtxt("limits/dark_photon/dark_photon_limits_jsns.txt", delimiter=",")
        mlist = saved_limits[:,0]
        eplist = saved_limits[:,1]
    else:
        # Binary search.
        print("Running dark photon limits...")
        outlist = open("limits/dark_photon/dark_photon_limits_jsns.txt", "w")
...
if __name__ == "__main__":
    ...
    plt.savefig("plots/dark_photon/spectra/jsns_timing_signal.png")
    plt.clf()
    dm_signal = EventsGen2((75, 0.001))
    energies = n_meas[:, 0] / pe_per_mev
    plt.plot(energies,dm_signal, drawstyle="steps-mid")
    plt.savefig("plots/dark_photon/spectra/jsns_energy_signal.png")
    plt.clf()
