...
def EventsGenerator(nsi_array, expo, flux, osc_factory):
  ...
  zenith_arr = np.round(np.linspace(min_z,max_z,n_z), decimals=3)
  energy_arr = np.array([1884.00, 2113.00, 2371.00, 2661.00, 2985.00,
                         3350.00, 3758.00, 4217.00, 4732.00, 5309.00,
...
def FlatPrior(cube, ndim, nparams):
  ...
  cube[7] = 0.5 * pi * (2 * cube[7] - 1)  # emu phase
  cube[8] = 0.5 * pi * (2 * cube[8] - 1)  # emu phase
...
def main():
  ...
  # Prepare flux.
  z_bins = np.round(np.linspace(min_z, max_z, n_z), decimals=3)
  flux_list = []
  ...
  do_plots = False
  if do_plots == True:
    e_bins = np.array([1884.00, 2113.00, 2371.00, 2661.00, 2985.00,
                       3350.00, 3758.00, 4217.00, 4732.00, 5309.00,
                       5957.00, 6683.00, 7499.00, 8414.00, 9441.00])
    z_bins = np.linspace(-1, 1, n_z + 1)
    nsi1 = EventsGenerator([0.2, 0, 0.1, 0, 0, 0.2, 1.5 * pi], exposure, flux_list, osc_factory)
    ...
    ticks = np.linspace(0, 500, 6)

    # Plot SM
    ax = fig.add_subplot(gs[0, 0], polar=True)
    ax.set_theta_zero_location('N')
    plt.rgrids((0, 2000, 4000, 6000, 8000), color='w')
    ...
    plt.savefig("plots/rates/dune/png/polar_plot_hyperk_atmos.png")
    plt.savefig("plots/rates/dune/pdf/polar_plot_hyperk_atmos.pdf")
  ...
  file_string = "all_complex_nsi_hyperk_atmos_mu"
